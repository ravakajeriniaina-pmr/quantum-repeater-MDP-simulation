...
def swap_werner(w1: float, w2: float, decay: float) -> float:
    """
    Werner parameter after swap of two Werner states.

    Args:
        w1: Werner parameter of link 1 (already accounting for decoherence)
        w2: Werner parameter of link 2 (already accounting for decoherence)
        decay: Additional decay factor (default 1.0).
               WARNING: do NOT pass exp(-|age1-age2|/t_coh) here —
               that term is physically incorrect. Use decay=1.0
               and pre-compute w1, w2 with correct decoherence.
...
def swap_werner_from_ages(age1: int, age2: int, w0: float,
                          t_coh: float) -> float:
    """
    Werner parameter after entanglement swap.

    Physical derivation:
        w1 = w0 * exp(-age1 / t_coh)
        w2 = w0 * exp(-age2 / t_coh)
        w_out = w1 * w2 = w0^2 * exp(-(age1 + age2) / t_coh)

    Delegates to werner_utils.swap_output_werner for consistency.
    """
    if np.isinf(t_coh):
        return w0 * w0
    return w0 * w0 * np.exp(-(age1 + age2) / t_coh)
...
def max_useful_age(w0: float, t_coh: float) -> int:
    """
    Maximum age 'age' such that a symmetric swap (age, age)
    still produces positive SKR.

    For symmetric swap: w_out = w0^2 * exp(-2*age/t_coh)
    Condition: w_out >= w_threshold (0.7476)
    → age <= -t_coh/2 * ln(w_threshold / w0^2)

    Note: for asymmetric swaps (a1 != a2), the relevant
    quantity is (a1+a2)/2, not max(a1,a2).
    """
    w_threshold = 0.7476
...
def swap_werner_table(w0: float, t_coh: float,
    ...
    """Precomputed table using correct formula."""
    size = max_age + 1
    if np.isinf(t_coh):
        return np.full((size, size), w0 * w0)

    ages = np.arange(size, dtype=np.float64)
    sum_ages = ages[:, None] + ages[None, :]   # ← somme, pas max
    return w0 * w0 * np.exp(-sum_ages / t_coh)
...
if __name__ == "__main__":
    ...
    assert abs(w_sym - w_asym1) < 1e-10, \
    "ages=(100,100) and ages=(0,200) must give same result (sum formula)"
    print(f"   ages=(100,100)={w_sym:.6f} == ages=(0,200)={w_asym1:.6f}  ✓  "
      f"(only total age matters)")
    print(f"   ages=(0,200) = {w_asym1:.6f}  ==  "
          f"ages=(200,0) = {w_asym2:.6f}  ✓  (symmetric)")

    # Verify: ages=(0,200) == ages=(100,100)
    # Both give exp(-(0+200)/1000) = exp(-(100+100)/1000) = exp(-0.2)
    # Equal because only the SUM of ages matters, not their distribution.
    # La distribution des âges n'a pas d'importance, seule la somme compte
    print(f"   ages=(100,100)={w_sym:.4f} == ages=(0,200)={w_asym1:.4f}  ✓  "
      f"(only total age matters, not distribution)")
    ...
    for a1, a2 in [(0, 0), (50, 100), (100, 50), (200, 200), (0, 500)]:
        ...
        w_manual = 0.95**2 * np.exp(-(a1 + a2) / 500)
